...
public class TickDataDao {
	...
	public TimeSeries getTickData(String symbol, long startTime, long endTime){
		...
		boundStmt.setString(0, symbol);
		boundStmt.setDate(1, LocalDate.fromMillisSinceEpoch(startTime));
		boundStmt.setDate(2, LocalDate.fromMillisSinceEpoch(endTime));
		ResultSet resultSet = session.execute(boundStmt);
		Iterator<Row> iterator = resultSet.iterator();
		DoubleArrayList values = new DoubleArrayList();
		LongArrayList dates = new LongArrayList();
	...
	public void insertTimeSeries(TimeSeries timeSeries) throws Exception{
		logger.info(String.format("Clustering.insertTimeSeries : %s - Begin inserting %s rows", timeSeries.getSymbol(), timeSeries.getDates().length));
...
//		int timeSeriesLength = dates.length;
		...
		for (int i = 0; i < dates.length; i++){
			...
			boundStmt.setString(0, symbol);
			boundStmt.setTimestamp(1, new Date(dates[i]));
			boundStmt.setDouble(2, values[i]);
			session.execute(boundStmt);
			TOTAL_POINTS.incrementAndGet();
			//logger.info(String.format("Clustering.insertTimeSeries : Inserted %s out of %s", i, timeSeriesLength));
		}
		return;
	}
